// TUI Music Bot – single-file, prefix commands
...
async function search(query) {
  const node = shoukaku.getNode();
  const q = isUrl(query) ? query : `ytsearch:${query}`; // LavaSrc handles many providers if installed
...
async function playNext(guildId) {
  ...
  if (channel && channel.isTextBased()) {
    channel.send(`▶️ Now playing: **${next.info.title}**`)
      .catch(() => {});
  }
}
...
client.on('messageCreate', async (msg) => {
  ...
  try {
    if (name === 'p' || name === 'play') {
      ...
      if (!query) return msg.reply(`Usage: \`${PREFIX}play <query|url>\``);
